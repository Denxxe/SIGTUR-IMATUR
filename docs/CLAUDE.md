...
| 073 | `073_bono_vacacional_calculado.sql` | ✅ Ejecutado (2026-08-27) | **Bono Vacacional: de captura manual a cálculo (fase N-D).** Las primas, el sueldo normal diario y la alícuota pasan por `Nomina::calcular()` — el mismo motor de la quincenal, así que los dos documentos no pueden discrepar. `BonoVacacional::TIPOS = Nomina::TIPOS` (5 tipos) y `tipoPersonal()` delega, para que un trabajador caiga en la misma hoja en ambos. Los días se cuentan **a la fecha de corte** del período, no a hoy. ⚠️ **El TOTAL sigue siendo de captura**: su fórmula NO está en ninguna fuente (la plantilla documenta la *alícuota*, no el monto pagado, y el mes ya calculado que el cliente prometió no llegó). En vez de inventarla se agregó `total_calculado` con la estimación bajo un supuesto explícito (`diario × días`) **al lado** de `total_bono_vacacional`, y la UI y el export muestran la **diferencia** — así la pregunta se responde sola en cuanto llegue un mes real. `recalcular()` **preserva los totales confirmados**; `aceptarCalculados()` los toma en bloque (auditado). El período congela cesta ticket y tasa, y exige el mes cargado en `nomina_parametros_mes`. Columnas nuevas de entradas (grado, %, años) para auditar el número, y `advertencias` por empleado. `tipo_personal` VARCHAR(20)→(25). |
| 074 | `074_nomina_tasa_origen.sql` | ✅ Ejecutado (2026-09-13) | **Origen de la tasa del dólar.** `nomina_parametros_mes` +`tasa_fuente` (BCV/Manual, CHECK) +`tasa_fecha_valor` +`tasa_consultada_at`. El botón *Consultar BCV* de `/nomina/parametros` trae la tasa oficial y la **propone**; Talento Humano confirma. `NominaController::consultarTasa()` (JSON, rol 1/2) devuelve la sugerencia y **no guarda nada**; `guardarParametros()` marca `BCV` solo si el valor guardado coincide con el sugerido (tolerancia 0,00005) — si el usuario lo retoca queda `Manual` y se limpian fecha valor/consulta. ⚠️ **La consulta ocurre SOLO al cargar el parámetro**: `Nomina::generarPeriodo()`/`recalcular()` nunca salen a internet, usan la tasa congelada en `nomina_periodos`. Pregunta abierta **N-4**: no está confirmado que la tasa aplicada sea la del BCV del día (la plantilla del cliente trae 36,58 y 36,23 en hojas distintas del mismo período). |
...
> **Fuente única de verdad (regenerado 2026-08-04, ampliado con las mig. 069-073 el 2026-08-27):** `database/schema_consolidado.sql` es **autosuficiente**: contiene el esquema base + **todas** las migraciones **001–073** (**60 tablas**) + los catálogos institucionales sembrados + un usuario administrador de arranque. Generado desde la BD viva con `pg_dump --no-owner --no-privileges` excluyendo los datos operativos, y **verificado cargándolo en una base vacía** (2026-08-27: 60 tablas, 25 ubicaciones, 24 feriados, catálogos de nómina, 0 errores).
>
...
21b. **Baja de un bien ≠ borrado (y el reporte de bajas se equivoca — H-16)** — dar de baja pone `estatus = 'Dado de baja'` **conservando `is_active = TRUE`**: el bien sale del inventario activo (`Inventario::fueraDeInventario()`, `ESTATUS_FUERA_DE_INVENTARIO`) pero su registro se preserva como aval (B-38). `is_active = FALSE` es **la papelera**, para registros creados por error. Para listar desincorporados usar **`Inventario::desincorporados()`**, nunca `is_active = FALSE`. `ReportesInventarioTrait::bajasInventario()` **todavía consulta la papelera** y por eso no muestra ninguna desincorporación real — ver `docs/BACKLOG.md` §4 H-16. Todas las consultas de inventario activo deben excluir el estatus de baja (`estatus <> 'Dado de baja'`, como ya hacen Dashboard, CMI, Centro de Alertas y el reporte de inventario).
...
> **Nota:** `database/schema_consolidado.sql` es autosuficiente (001–073). `database/migrations/` solo sirve como historial y para **actualizar** instalaciones antiguas, no para instalar desde cero. (`schema_completo.sql` y `schema.sql` fueron **eliminados** en 2026-08-04: cubrían hasta la 011 y el base original, y solo generaban confusión sobre cuál importar. Recuperables desde el historial de git si hicieran falta.)
...
| Schema consolidado (instalar desde cero) | `database/schema_consolidado.sql` — **autosuficiente, 001-073 + catálogos + admin de arranque; no aplicar migraciones encima** |

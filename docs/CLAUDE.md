...
**Última actualización:** 2026-07-12 (migraciones hasta 058, todas ejecutadas en este entorno; recuperación de contraseña por correo (autoservicio) + anti-duplicado de correo en Empleados + remitente = correo institucional; notificaciones "vistas" por usuario en la campana; auditoría de reportes/listados completa (RRHH+Rutas+Visitantes+Pasantes+Inventario+transversal); tolerancia de salida anticipada en asistencia; ver `docs/BACKLOG.md` para pendientes — suite `php tests/run.php` 18/18 ✓)  

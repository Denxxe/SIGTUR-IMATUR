...
**Centro de alertas / campana (header):** la fuente única es `CentroAlertas::resumen($rol)` (model), reutilizada por el reporte `reportes/alertas` **y** por la campana de notificaciones en `header.php` (dropdown role-aware con badge de conteo accionable). Para agregar/editar una alerta, tocar **solo** `CentroAlertas::resumen()`. Se ejecuta en cada página (header) — mantener sus consultas baratas. `ReportesController::alertas()` ya no contiene la lógica (solo renderiza).

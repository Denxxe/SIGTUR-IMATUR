...
**Última actualización:** 2026-07-10 (migraciones hasta 055 — 054/055 pendientes de aplicar en este entorno; UX: duración de permisos/reposos autocalculada por fechas; fix bitácora de accesos/IP + bitácora general solo-Admin + fix pérdida silenciosa de permisos de rol; cabecera institucional (logos+RIF) unificada en Dossier de Taller, PDF de Permisos y Reposos añadido; ver `docs/BACKLOG.md` para pendientes — suite `php tests/run.php` 18/18 ✓)  
...
> **Nota (2026-06-29):** migraciones aplicadas hasta la **053**. Para qué falta y decisiones pendientes, ver siempre `docs/BACKLOG.md` (fuente única de seguimiento).
...
| Schema consolidado (instalar desde cero) | `database/schema_consolidado.sql` (001-023 + seeds; aplicar 024-053 encima) |

...
**Documentos privados — almacenamiento y descarga:** los recaudos de expediente y los documentos de pasantes se guardan **fuera del web root** en `storage/uploads/{expedientes,pasantes}/` (no accesibles por URL). Se sirven **solo** vía `DescargaController::expediente($idDoc)` (rol 1,2) / `::pasante($idDoc)` (rol 1,3), que resuelven el archivo por **id de registro** (no por ruta: usan `basename()` del valor guardado → sin path traversal), validan rol e `is_active`, y hacen stream con `Content-Disposition: inline`. Las vistas enlazan a `/descarga/expediente/{id}` y `/descarga/pasante/{id}`. La subida valida extensión **y MIME real** (`mime_content_type`). `DescargaController` está en `$accesoSiempre` del Router (hace su propio chequeo de rol). **Las evidencias de talleres siguen en `public/uploads/talleres/`** (baja sensibilidad). Nuevos uploads guardan solo el nombre en `archivo_url`; los valores antiguos (`/uploads/...`) siguen funcionando por el `basename()`.

...
{
    // Tipo de falta (mig.048): inasistencia injustificada o incumplimiento de reglas.
    const TIPOS = ['Inasistencia injustificada', 'Incumplimiento de reglas'];
    const TIPO_DEFAULT = 'Inasistencia injustificada';
    ...
    public static function save(array $data, $user_id = null) {
        ...
        $tipo = in_array($data['tipo'] ?? '', self::TIPOS, true) ? $data['tipo'] : self::TIPO_DEFAULT;
        if ($id) {
            $previos = self::find($id);
            $db->query("UPDATE faltas SET fecha=:fecha, motivo=:motivo, tipo=:tipo, updated_at=CURRENT_TIMESTAMP, updated_by=:uid WHERE id=:id");
            $db->bind(':id', $id);
        } else {
            $previos = null;
            $db->query("INSERT INTO faltas (id_empleado, fecha, motivo, tipo, created_by) VALUES (:emp, :fecha, :motivo, :tipo, :uid) RETURNING id");

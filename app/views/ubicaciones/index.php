...
<div class="sig-table-wrap anim-slide-up">
    <table class="sig-table">
        <thead><tr><th>ID</th><th>Nombre de Sede/Almacén</th><th>Referencia</th><th class="col-actions">Acciones</th></tr></thead>
        <tbody>
...
<div class="modal fade" id="modalUbi" tabindex="-1">
    <div class="modal-dialog">
        <form action="<?php echo URL_ROOT; ?>/ubicaciones/store" method="POST" class="modal-content">
            <div class="modal-header"><h5 class="modal-title" id="modalUbiLabel">Ubicación Física</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
...
<script>
    function nuevaUbi() { document.getElementById('modalUbiLabel').innerText='Nueva Ubicación'; document.getElementById('ubi_id').value=''; document.querySelector('#modalUbi form').reset(); }
    function editarUbi(ubi) { document.getElementById('modalUbiLabel').innerText='Editar: '+ubi.nombre; document.getElementById('ubi_id').value=ubi.id; document.getElementById('ubi_nombre').value=ubi.nombre; document.getElementById('ubi_descripcion').value=ubi.descripcion; new bootstrap.Modal(document.getElementById('modalUbi')).show(); }
</script>

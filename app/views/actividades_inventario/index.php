...
<div class="page__head anim-slide-up">
    <div class="page__title-block">
        ...
        <h1 class="page__title"><?php echo $data['titulo'] ?? 'Inventario de Actividades'; ?></h1>
    ...
    <table class="sig-table">
        ...
        <tbody>
            <?php if (empty($data['actividades'])): ?>
                <tr>
                    <td colspan="6" class="sig-table-empty">No hay movimientos registrados.</td>
                </tr>
            <?php else: ?>
    ...
    <div class="modal-dialog">
        <form action="<?php echo URL_ROOT; ?>/actividadesinventario/store" method="POST" class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalMovimientoLabel">Registrar Movimiento</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                ...
                <div class="sig-field mb-3"><label class="sig-field__label">Bien Nacional <span class="req">*</span></label>
                    <select name="id_inventario" id="mov_inventario" class="sig-select" required>
                        <option value="">Seleccione...</option><?php foreach ($data['inventario'] ?? '' as $inv): ?><option value="<?php echo $inv->id; ?>"><?php echo $inv->codigo_bn . ' - ' . $inv->nombre; ?></option><?php endforeach; ?>
                    </select>
                </div>
                <div class="row g-3 mb-3">
                    <div class="col-md-6">
                        <div class="sig-field"><label class="sig-field__label">Fecha <span class="req">*</span></label><input type="date" name="fecha" id="mov_fecha" value="<?php echo date('Y-m-d'); ?>" class="sig-input" required></div>
                    </div>
                    <div class="col-md-6">
                        <div class="sig-field"><label class="sig-field__label">Operación <span class="req">*</span></label><select name="tipo_movimiento" id="mov_tipo" class="sig-select" required>
                ...
                <div class="sig-field mb-3"><label class="sig-field__label">Responsable (Opcional)</label>
                    <select name="id_empleado_responsable" id="mov_empleado" class="sig-select">
                        <option value="">N/A</option><?php foreach ($data['empleados'] ?? '' as $emp): ?><option value="<?php echo $emp->id; ?>"><?php echo $emp->nombre . ' ' . $emp->apellido; ?></option><?php endforeach; ?>
                    </select>
                </div>

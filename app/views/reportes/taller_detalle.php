...
<div class="page__head anim-slide-up d-print-none">
    ...
    <div class="page__actions">
        <div style="display:flex; gap:var(--sp-2);">
            <a href="<?php echo URL_ROOT; ?>/reportes/exportarDossierCsv/<?php echo $data['taller']->id; ?>" class="btn-sig btn-sig--ghost btn-sig--sm">
            ...
            <div class="row g-4">
                <div class="col-md-7" style="border-right:1px solid var(--border-subtle);">
                    <div style="margin-bottom:var(--sp-6);">
                        ...
                        <h2 style="font-size:24px; font-weight:800; color:var(--brand-600); line-height:1.2; margin:0;"><?php echo $data['taller']->nombre; ?></h2>
                    </div>
                    <div class="row g-4">
                        <div class="col-6">
                            ...
                            <div style="font-weight:700; color:var(--text-primary);"><?php echo $data['taller']->fac_nom . ' ' . $data['taller']->fac_ape; ?></div>
                        </div>
                        <div class="col-6">
                ...
                <div class="col-md-5">
                    <div style="display:grid; grid-template-columns:repeat(2, 1fr); gap:var(--sp-4);">
                        <div style="background:var(--bg-muted-subtle); padding:var(--sp-4); border-radius:var(--r-md); text-align:center;">
                            ...
                            <div style="font-weight:800; font-size:15px; color:var(--text-primary);"><?php echo date('d/m/Y', strtotime($data['taller']->fecha_inicio)); ?></div>
                        </div>
                        <div style="background:var(--bg-muted-subtle); padding:var(--sp-4); border-radius:var(--r-md); text-align:center;">
                        ...
                        <div style="grid-column: span 2; background:var(--brand-50); padding:var(--sp-4); border-radius:var(--r-md); text-align:center;">
                            ...
                            <span class="sig-badge sig-badge--brand" style="font-size:13px;"><?php echo $data['taller']->estado; ?></span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <?php if($data['informe']): ?>
                        ...
                        <div style="display:grid; grid-template-columns:repeat(4, 1fr); gap:var(--sp-2); text-align:center; padding-top:var(--sp-4); border-top:1px solid var(--border-subtle);">
                            <div>
                                ...
                                <span style="font-weight:800; color:var(--brand-600);"><?php echo $data['informe']->mujeres; ?></span>
                            </div>
                            <div>
                                <small style="display:block; font-size:9px; font-weight:700; color:var(--text-tertiary); text-transform:uppercase;">Hombres</small>
                                <span style="font-weight:800; color:var(--text-secondary);"><?php echo $data['informe']->hombres; ?></span>
                            </div>
                            <div>
                                <small style="display:block; font-size:9px; font-weight:700; color:var(--text-tertiary); text-transform:uppercase;">Niños/as</small>
                                <span style="font-weight:800; color:var(--success-600);"><?php echo (int)$data['informe']->ninas + (int)$data['informe']->ninos; ?></span>
                            </div>
                            <div style="border-left:1px solid var(--border-subtle);">
                                ...
                                <span style="font-weight:900; color:var(--text-primary); font-size:16px;"><?php echo $data['informe']->total_atendidas; ?></span>
                            </div>
                        </div>
                    <?php else: ?>
            ...
            <div class="sig-card h-100">
                ...
                <div class="sig-card__body" style="padding:var(--sp-6);">
                    <div style="font-size:14px; color:var(--text-secondary); line-height:1.7; margin-bottom:var(--sp-6); min-height:100px;">
                        <?php echo $data['informe'] ? nl2br(htmlspecialchars($data['informe']->resumen_actividad)) : '<p style="font-style:italic; opacity:0.6;">Sin resumen ejecutivo disponible.</p>'; ?>
                    </div>
            ...
            <table class="sig-table">
                ...
                <tbody>
                    <?php if(empty($data['participantes'])): ?>
                        <tr><td colspan="4" class="sig-table-empty">No se han registrado participantes en este taller.</td></tr>
                    <?php else: ?>
                        <?php foreach($data['participantes'] as $p): ?>
                        <tr>
                            <td class="cell-id" style="padding-left:var(--sp-8);"><?php echo $p->cedula; ?></td>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:100px; text-align:center;">
                <div>
                    <div style="width:240px; border-top:1px solid var(--text-primary); margin:0 auto 12px; padding-top:12px;">
                        <div style="font-weight:800; font-size:14px; text-transform:uppercase;"><?php echo $data['taller']->fac_nom . ' ' . $data['taller']->fac_ape; ?></div>
...
@media print {
    ...
    header, aside, .d-print-none, .page__head { display: none !important; }
    .print-area { width: 100% !important; margin: 0 !important; padding: 0 !important; }
    .sig-card { border: 1px solid #ddd !important; box-shadow: none !important; break-inside: avoid; }
    .sig-table th { background: #f8f9fa !important; color: #333 !important; }
}
</style>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new ApexCharts(document.querySelector("#chartDemografia"), {
        ...
        legend: { show: false },
        stroke: { show: false },
        plotOptions: { pie: { donut: { size: '70%', labels: { show: true, total: { show: true, label: 'TOTAL', color: textPrimary, fontSize: '12px', fontWeight: 800 } } } } },

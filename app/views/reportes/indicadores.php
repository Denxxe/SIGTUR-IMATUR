                ...
                <table class="sig-table">
                    ...
                    <tbody>
                        <?php foreach ($data['empPorDepto'] ?? [] as $e): ?>
                            <tr>
                                <td class="cell-strong"><?php echo $e->departamento ?? 'Sin Dpto'; ?></td>
                                <td style="text-align:center; font-weight:700; color:var(--brand-600);"><?php echo $e->total ?? 0; ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                ...
                <table class="sig-table">
                    ...
                    <tbody>
                        <?php foreach ($data['invPorCat'] ?? [] as $i): ?>
                            <tr>
                                <td class="cell-strong"><?php echo $i->categoria ?? 'Sin Cat'; ?></td>
                                <td style="text-align:center; font-weight:700; color:var(--success-600);"><?php echo $i->total ?? 0; ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        new ApexCharts(document.querySelector("#chartEmpDepto"), {
            ...
            series: [{
                name: 'Empleados',
                data: <?php echo json_encode($valD); ?>
            }],
            xaxis: {
                categories: <?php echo json_encode($lblD); ?>,
                labels: {
                    style: {
                        colors: textPrimary,
                        fontSize: '11px'
        ...
        foreach ($data['invPorCondicion'] ?? [] as $c) {
            $totalInv += (int)($c->total ?? 0);
        }
        foreach ($data['invPorCondicion'] ?? [] as $c) {
            $lblCond[] = $c->condicion ?? 'Desconocida';
            $valCond[] = $totalInv > 0 ? round(((int)($c->total ?? 0) / $totalInv) * 100) : 0;
        }
        ?>
        ...
        new ApexCharts(document.querySelector("#chartTallMes"), {
            ...
            series: [{
                name: 'Talleres',
                data: <?php echo json_encode($valM); ?>
            }],
            xaxis: {
                categories: <?php echo json_encode($lblM); ?>,
                labels: {
                    style: {
                        colors: textPrimary
                    }
